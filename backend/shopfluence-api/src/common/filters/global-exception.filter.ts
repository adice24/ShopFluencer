...
/** Prisma errors can fail `instanceof` if multiple client copies exist. */
function prismaErrorCode(exception: unknown): string | undefined {
  ...
  for (const key of ['code', 'errorCode'] as const) {
    const v = e[key];
    if (typeof v === 'string' && /^P\d+$/.test(v)) return v;
  }
  return undefined;
}
...
export class GlobalExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(GlobalExceptionFilter.name);
    ...
    if (exception instanceof HttpException) {
      ...
      const code: string =
        prismaErrorCode(exception) ?? err.code ?? 'UNKNOWN';
      this.logger.error(
        `Prisma Error [${code}]: ${err.message ?? String(exception)}`,
        err.stack,
      );
      errorCode = `DB_ERROR_${code}`;
      message = 'A database error occurred.';

      if (code === 'P1001') {
        status = HttpStatus.SERVICE_UNAVAILABLE;
        message =
          'Database is unreachable. Check DATABASE_URL (use Supabase pooler :6543 if direct :5432 fails), resume the project, or use ADMIN_EMAIL / ADMIN_PASSWORD_HASH for admin login.';
      ...
      const err = exception as any;
      this.logger.error(`Prisma Validation Error: ${err.message}`, err.stack);
      status = HttpStatus.BAD_REQUEST;
