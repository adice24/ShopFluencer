...
export class LinksController {
    ...
    @Roles(UserRole.INFLUENCER)
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Get all links for the current influencer' })
...
export class PublicRedirectController {
    ...
    @Get(':shortCode')
    @ApiOperation({ summary: 'Resolve short link and 302 redirect to destination' })
    async redirect(
    ...
    @ApiOperation({ summary: 'Resolve short link and return destination URL as JSON' })
    async resolveJson(
        @Param('shortCode') shortCode: string,
        @Req() req: Request,
    ) {
