...
export class StorefrontService {
    ...
    private async getInfluencerId(userId: string, role: string, providedInfluencerId?: string): Promise<string> {
    ...
    private async checkStorefrontOwnership(influencerId: string, storefrontId: string) {
        ...
        if (storefront.influencerId !== influencerId) {
            throw new ForbiddenException('You do not have permission to modify this storefront');
        }
        return storefront;
    }
    ...
    async getBySlug(slug: string) {
                    ...
                    include: {
                        product: {
                            include: {
                                ...
                                variants: { where: { isActive: true, stock: { gt: 0 } }, orderBy: { sortOrder: 'asc' } },
                            },
                        },
                    },
                },
        ...
        return {
            ...
            meta: {
                ...
                description: storefront.metaDescription || storefront.description || `Shop ${storefront.title}'s curated products`,
    ...
    async upsert(userId: string, role: string, data: UpsertStorefrontDto) {
        const influencerId = await this.getInfluencerId(userId, role, data.influencerId);

        const existingWithSlug = await this.prisma.storefront.findUnique({ where: { slug: data.slug } });
        if (existingWithSlug && existingWithSlug.influencerId !== influencerId) {
            throw new ConflictException('Slug is already taken by another storefront');
        }
    ...
    async setStatus(storefrontId: string, status: StorefrontStatus) {
        const storefront = await this.prisma.storefront.update({
            ...
            data: {
                status,
                publishedAt: status === StorefrontStatus.PUBLISHED ? new Date() : undefined,
            },
        });
     ...
     */
    async addProduct(userId: string, role: string, storefrontId: string, productId: string, customNote?: string) {
     ...
     */
    async removeProduct(userId: string, role: string, storefrontId: string, productId: string) {
        if (role !== UserRole.ADMIN) {
            const influencerId = await this.getInfluencerId(userId, role);
            await this.checkStorefrontOwnership(influencerId, storefrontId);
        }
