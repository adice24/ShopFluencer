...
export class AnalyticsService {
     ...
     */
    async getInfluencerDashboard(influencerId: string, dateFrom?: string, dateTo?: string) {
    ...
    async getPlatformAnalytics(dateFrom?: string, dateTo?: string) {
        ...
        const where: Prisma.AnalyticsEventWhereInput = Object.keys(dateFilter).length
        ...
        return {
            summary: {
                ...
                platformConversionRate: totalStorefrontViews > 0
    ...
    async aggregateDaily(date: Date) {
        ...
        for (const group of influencerEvents) {
            ...
            const views = await this.prisma.analyticsEvent.count({
                where: { ...dateFilter, influencerId: group.influencerId, eventType: EventType.STOREFRONT_VIEW },
            });
            ...
            const clicks = await this.prisma.analyticsEvent.count({
                where: { ...dateFilter, influencerId: group.influencerId, eventType: EventType.PRODUCT_CLICK },
            });

            const orders = await this.prisma.affiliateConversion.count({
                where: { influencerId: group.influencerId, ...dateFilter, status: 'CONFIRMED' },
            });
        ...
        this.logger.log(`Daily analytics aggregated for ${startOfDay.toISOString().slice(0, 10)}`);
    }
    ...
    async getSuggestions(influencerId: string) {
        ...
        if (profile && !profile.bio) {
            suggestions.push({
                type: 'PROFILE',
                title: 'Add a Bio',
                description: 'A compelling bio increases trust. Tell your audience why you recommend these products.',
        ...
        if (suggestions.length === 0) {
            suggestions.push({
                ...
                description: 'You are doing great! Share your link on new platforms to get more traffic.',
